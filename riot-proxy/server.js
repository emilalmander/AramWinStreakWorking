...
app.get("/match-history/:puuid", async (req, res) => {
  ...
  console.log(`Fetching match history for PUUID: ${puuid}, Start: ${start}, Count: ${count}`);
  try {
...
app.get("/match-timeline/:matchId", async (req, res) => {
  const { matchId } = req.params;
  try {
    const url = `https://europe.api.riotgames.com/lol/match/v5/matches/${matchId}/timeline?api_key=${API_KEY}`;
    const response = await axios.get(url);
    res.json(response.data);
  } catch (error) {
    const status = error.response?.status || 500;
    const message = error.response?.data?.status?.message || "Unknown error";
    console.error(`Error fetching match timeline: ${message} (status: ${status})`);
    res.status(status).json({ error: message });
  }
});
